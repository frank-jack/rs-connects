                ...
                if modelData.profile.isAdmin {
                    ...
                    .alert("Make Admin", isPresented: $showAdminAlert, actions: {
                        Button("Make Admin", role: .destructive, action: {
                            modelData.putUserData(profile: Profile(id: profile.id, email: profile.email, phone: profile.phone, username: profile.username, image: profile.image, isAdmin: true, token: mainToken))
                        })
                    }, message: {
                        ...
                        .onChange(of: selectedItem) { newItem in
                            Task {
                                if let data = try? await selectedItem?.loadTransferable(type: Data.self) {
                                    modelData.putUserData(profile: Profile(id: profile.id, email: profile.email, phone: profile.phone, username: profile.username, image: UIImage(data: data) ?? UIImage(imageLiteralResourceName: "ProfilePic"), isAdmin: profile.isAdmin, token: mainToken))
                                }
                            }
                        }
                    } else {
